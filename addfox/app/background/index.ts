  ...
  (
    ...
    if (msg.type === "CAPTURE_TAB") {
      chrome.storage.local.get([SCREENSHOT_KEY], (r) => {
        ...
        chrome.tabs.captureVisibleTab(null, { format: "png" }).then(
          (dataUrl) => {
...
function getScreenshots(): { dataUrl: string; at: number }[] {
  return []; // sync fallback; real list from storage in GET_SCREENSHOTS
}
